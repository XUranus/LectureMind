...
The fallback chain ensures the system always returns a useful response. Fast RAG falls back when vector retrieval cannot find sufficiently relevant documents (minimum relevance threshold \texttt{MIN\_\allowbreak RELEVANCE\_\allowbreak THRESHOLD=0.3}, minimum document count \texttt{MIN\_\allowbreak DOCUMENTS\_\allowbreak REQUIRED=2}). Agentic RAG falls back to Fast RAG on agent-level failures and ultimately to LLM Direct if retrieval also fails.
...
\begin{table}[H]
    \centering
    \caption{Agent Tools}
    \begin{tabular}{@{}p{4.2cm}p{5.8cm}p{2.5cm}@{}}
        \toprule
        \textbf{Tool} & \textbf{Purpose} & \textbf{Input} \\ \midrule
        \texttt{search\_knowledge} & Vector search on knowledge points \& transcripts & query string \\
        \texttt{search\_slides}    & OCR-text search on slide content                & query string \\
        \texttt{get\_section\_details} & Full section content & section order \\
        \texttt{get\_lecture\_summary} & Overview + chapters & --- \\
        \texttt{list\_sections} & All section titles/times & --- \\
        \texttt{get\_transcript\_at\_time} & Transcript slice & start, end times \\ \bottomrule
    \end{tabular}
\end{table}
